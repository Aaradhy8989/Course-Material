current = linspace(0.001,1,10000);
frequency = zeros(size(current));
for i=1:10000
    tracker=0;
    count=0;
    gkmax=.36;
    ...
    for iter=1:niter
      ...
      if vhist(iter)>=0 && tracker==0
          count=count+1;
          tracker=1;
      elseif vhist(iter)<0 && tracker==1
          tracker=0;
      end
    end
    frequency(i) = count/100;
    
end
frequency;
plot(current,frequency)
title('Frequency vs Applied Current (mA)')